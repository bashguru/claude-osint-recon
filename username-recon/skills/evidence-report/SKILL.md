...
Turn confirmed findings into a defensible deliverable: a **single HTML file** that
...
**username-search**): **Assisted** — pause for the analyst to solve it in their own
browser (if they close the tab, record `waf` + "tab closed — not verified");
**Automated** — screenshot the block page as evidence, record `waf` + "bot
challenge — blocked, could not verify", and continue. Full detail:
...
1. **Scaffold a case file** (optional — you can also write the JSON directly):

   ```bash
   python3 "${CLAUDE_PLUGIN_ROOT}/skills/evidence-report/scripts/build_report.py" \
       --init evidence/case.json --case-id TCS-2026-014 --subject johndoe \
...
2. **Fill in `findings`** — one entry per captured account, with the screenshot
   ...
   (title, case_id, examiner, subject, authorization basis, opened).
...
A case header (case id, examiner, subject, authorization basis, dates), summary
...
- **Select all / None / Only "found" / Show relevant only** — quick selection and
  a focused view. Deselected cards dim, and are hidden when printed/exported to PDF.
- **Export CSV** — the selected findings as a spreadsheet (includes the SHA-256).
- **Export JSON (case file)** — the selected findings in *this plugin's own
...
Copy for Claude and paste it back to me — I'll write the summary."* When they do,
...
- **Word (.docx) or PDF** — feed the findings (and, if wanted, the screenshots)
