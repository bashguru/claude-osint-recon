...
- **Playwright (fallback).** A self-contained browser Claude runs on its own. Add it
  ...
  *visible* so you can solve any human-checks. See https://github.com/microsoft/playwright-mcp
...
- **Mac.** Usually already there. If not, see https://www.python.org/downloads/ or `brew install python`.
